...
def build_operator_handoff(session: SessionProjectConfig) -> dict[str, Any]:
    ...
    return {
        ...
        "color_prep": {
            ...
            "layout": "compact",
            "audio_layout": "A1 master-audio; A2+ linked camera scratch audio for sync QA",
        },
        "next_steps": [
            ...
            "Use A1 as final/reference audio; A2+ scratch tracks are linked sync-QA tracks.",
...
def render_operator_handoff_markdown(payload: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "4. Use A1 as final/reference audio; use A2+ linked scratch tracks only for sync QA or repair.",
            "5. Go to the Color page and grade each source angle timeline item with Local Grades.",
            "6. Match angles within each take first: white keys, black piano finish, gold plate, skin when visible, and window highlights.",
            "7. Grab Gallery Stills for useful same-angle starting points, then Apply Grade to the next take and adjust independently.",
            "8. Do not use Remote Grades, Shared Nodes, or CDL commands for the standard workflow.",
            "9. After color prep, duplicate the timeline before manual multicam conversion or downstream editing.",
            "10. Use final timeline grades only for light take-to-take finishing after the edit is locked.",
            "",
            "## Repo Boundary",
            "",
